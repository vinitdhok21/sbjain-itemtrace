...
export default function ReportLostPage() {
  ...
  const handleImageChange = (e) => {
    ...
    e.target.value = ''; // Reset input so same file can be selected if needed
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('Error reporting lost item:', err.message);
      setError(getFriendlyErrorMessage(err, 'An error occurred while submitting your lost item report.'));
      setSubmitting(false);
      setUploadProgress(0);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 sm:px-6 lg:px-8 space-y-6 animate-[fadeIn_0.3s_ease-out]">
      ...
      <div className="space-y-2">
        <span className="inline-block px-2.5 py-0.5 bg-rose-50 text-rose-600 border border-rose-100 rounded-full text-[10px] font-bold uppercase tracking-wider select-none">
          Report Form
        </span>
        <h1 className="text-3xl font-display font-black text-slate-900 tracking-tight">I Lost Something</h1>
        <p className="text-sm text-slate-500 font-medium">
          Tell us what you lost and where you last saw it around campus.
        </p>
      </div>
        ...
        {success ? (
          <div className="space-y-8 animate-[scaleIn_0.3s_ease-out]">
            ...
            <div className="flex flex-col items-center justify-center text-center space-y-3 py-6 border-b border-slate-100">
              ...
              <h3 className="text-2xl font-bold text-slate-800">Lost Item Reported!</h3>
            ...
            <div className="space-y-6">
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-400 font-medium">
                  We checked active found reports around campus. See if any match your description:
                </p>
              </div>
              ...
              {loadingMatches ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 text-center space-y-2 border border-slate-105 border-dashed rounded-2xl bg-slate-50/10">
                  ...
                  <p className="text-xs text-slate-400 max-w-xs leading-relaxed">
                    We will list matches here and notify you if a student reports finding a similar item!
                  </p>
                </div>
              ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-slate-600" htmlFor="location">
                  Location Lost <span className="text-rose-500">*</span>
                </label>
                <select
                  id="location"
                  required
              ...
              <div className="flex flex-wrap items-center gap-4">
                ...
                {imagePreviews.map((previewUrl, idx) => (
                  <div key={idx} className="relative w-24 h-24 bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden shadow-xs group">
                    <img
                      src={previewUrl}
                      alt={`Selected lost item preview ${idx + 1}`}
